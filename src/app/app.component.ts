...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    const localData = localStorage.getItem("AbrarQureshi");
    if(localData != null) {
  ...
  onDelete(item: Student) {
    ...
    if(isDelet) {
      ...
      localStorage.setItem('AbrarQureshi', JSON.stringify(this.studentList));
    }
  }
  onEdit(item: Student) {
  ...
  saveStudent() {
    debugger;
    const isLocalPresent = localStorage.getItem("AbrarQureshi");
    if (isLocalPresent != null) {
      ...
      localStorage.setItem('AbrarQureshi', JSON.stringify(oldArray));
    } else {
      const newArr = [];
      newArr.push(this.studentObj);
      this.studentObj.id = 1;
      this.studentList = newArr;
      localStorage.setItem('AbrarQureshi', JSON.stringify(newArr));
    }
    this.closeModel()
  }
}
